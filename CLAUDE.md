...
- **Hosting: Vercel** (owner clarified after the initial build). Book APIs run through
  Vercel functions in `api/`, which adapt the existing `netlify/functions/` handlers.
...
- Functions (`api/` Vercel entrypoints wrapping `netlify/functions/` handlers):
  `create-book-checkout.js`, `book-stripe-webhook.js`
...
- `vercel.json`: noindex/security headers + `/books`, `/book-admin`, and legacy
  `/.netlify/functions/*` rewrites.
- Docs/setup: `BOOK_STORE_SETUP.md`, `.env.example`, `.env` (gitignored),
  `scripts/set-vercel-env.sh` (`scripts/set-netlify-env.sh` is legacy).
...
## Env vars (Vercel) — see BOOK_STORE_SETUP.md §2
...
- The Stripe **book** webhook endpoint: `/api/book-stripe-webhook`, event
...
- Owner ran the SQL. Still to do (human-only): fill 5 real keys in `.env` + run
  `scripts/set-vercel-env.sh`; add Stripe webhook; verify Resend DNS; upload 2 private PDFs;
